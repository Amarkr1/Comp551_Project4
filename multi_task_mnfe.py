...
my_train = np.concatenate((my_train,cy_train,cy_train,cy_train),axis=1)
my_test = np.concatenate((my_test,cy_test,cy_test,cy_test),axis=1)
...
fy_train = np.concatenate((cy_train, fy_train,cy_train,cy_train),axis=1)
fy_test = np.concatenate((cy_test, fy_test,cy_test,cy_test),axis=1)
...
ny_train = np.concatenate((cy_train, cy_train,ny_train,cy_train),axis=1)
ny_test = np.concatenate((cy_test, cy_test,ny_test,cy_test),axis=1)


ex_train, ey_train = mnist_reader2.load_mnist('data/EMNIST', kind='train')
ex_test, ey_test = mnist_reader2.load_mnist('data/EMNIST', kind='test')
ex_train = np.asarray(ex_train)
ex_test = np.asarray(ex_test)
ex_train = np.reshape(ex_train,[-1,28,28,1])
ex_test = np.reshape(ex_test,[-1,28,28,1])
ex_train = ex_train.astype('float32')
ex_test = ex_test.astype('float32')
ex_train = re_scale(ex_train)
ex_test = re_scale(ex_test)
#nx_train=i2b(nx_train)
#nx_test=i2b(nx_test)#image to binary

ey_train = keras.utils.to_categorical(ey_train, num_classes)
ey_test = keras.utils.to_categorical(ey_test, num_classes)
ey_train = np.concatenate((cy_train, cy_train, cy_train, ey_train),axis=1)
ey_test = np.concatenate((cy_test, cy_test, cy_test, ey_test),axis=1)

kx_train = np.concatenate((mx_train, fx_train,nx_train,ex_train),axis=0)
ky_train = np.concatenate((my_train, fy_train,ny_train,ey_train),axis=0)
...
model.fit(kx_train,ky_train,batch_size=100,epochs=FLAGS.epochs,validation_data=(ex_test,ey_test),verbose=2)

score = model.evaluate(nx_test, ny_test, verbose=1)
print('NotMNIST Test loss:', score[0])
print('NotMNIST Test accuracy:', score[1])

score = model.evaluate(fx_test, fy_test, verbose=1)
print('fashionMNIST Test loss:', score[0])
print('fashionMNIST Test accuracy:', score[1])

score = model.evaluate(mx_test, my_test, verbose=1)
